...
The authoring craft and full machine contract live in the `author-backlog` skill and `references/SPEC-BACKLOG-TOOL-CONTRACT.md` (Part A) — reference them when proposing fixes.
...
**specReferences usage:** items implementing spec-defined behavior should point to the relevant docs; referenced files must exist; references should be specific (prefer `references/SPEC-CORE.md` over a generic `docs/`).
