...
- **Not an exhaustive flag reference** → `references/SPEC-CLI.md` is the complete CLI spec and
  `references/SPEC-BACKLOG-TOOL-CONTRACT.md` §A.7 is the machine-surface contract. This skill is
...
human renderer or `rauf.log`. Full contract: `references/SPEC-BACKLOG-TOOL-CONTRACT.md` §A.7.
...
- Full CLI spec (every command + flag): `references/SPEC-CLI.md`
- Machine-surface contract (events, `DerivedStatus` incl. `health` + `statusSchemaVersion`,
  ...
  `references/SPEC-BACKLOG-TOOL-CONTRACT.md` §A.7.2
- Status label/tone source of truth: `references/state-labels.ts`
