...
- **Loop no longer silently wastes a retry when an agent backgrounds
  ...
  verification block in `RAUF.md` and the ADDON/GREENFIELD templates now forbid
  deferring the exit signal behind an async completion notification and require
  emitting it within the same turn; and a no-signal genuine-retry whose output
  shows that pattern is now annotated in `rauf.log` with the likely cause instead
  of a bare retry line. (#125)
