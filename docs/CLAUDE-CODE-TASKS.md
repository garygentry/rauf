...
description: How rauf's backlog.json relates to Claude Code's native Tasks system.
---
...
| **Managed by**     | Human + rauf manager tool            | Claude Code agent                  |
...
backlog.json item "001: Implement user auth"
  └─► Loop runner picks it, marks in_progress
      └─► claude -p session starts
          ...
          └─► Session ends, agent outputs RAUF_DONE
...
Each rauf iteration spawns a fresh `claude -p` session. Setting a shared task list ID would cause:
...
When using rauf loops to develop the rauf tool itself, the same principles apply:

- `.rauf/backlog.json` defines what to build
