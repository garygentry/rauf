...
- **`ClaudeCliProvider`** (`claude-cli`) — the pre-existing Claude adapter. Stream-parses
  ...
  credentials.
- **`CliAgent`** — one config-driven engine behind *every* other agent. It implements only
  `id`, `displayName`, `execute`, and `validateCredentials`. The two omissions are
  load-bearing:
  - **No `checkUsage`** — the runner gates *all* Anthropic usage handling on `checkUsage`
...
is *gracefully absent* rather than an error (REQ-OBS-02).
...
- a **descriptor map** (`id → AgentDescriptor`) used to enumerate and probe agents
  *without* constructing anything or reading run config.
...
- **`registerProvider(id, factory)`** — the *existing* surface, unchanged signature. For
...
- **`registerAgent(descriptor)`** — the descriptor-aware path. Populates *both* maps.
...
with the project rule that *status reads files, not subprocesses*. Two descriptors
override it:
...
returned as *data* (`available: false` with a human-readable `detail`), never an
...
*inside* the sandbox cwd and unlinked in a `finally` block, so it never escapes rauf's
...
a `Result` so a malformed entry is an *expected error*, not a throw. Defaults: `args` →
...
It does **not** validate that the id is *known* — that is the consumer's job
...
3. **Gates usage** only when the adapter defines `checkUsage`; for adapters without it, a
   literal "rate limit" string in output is *not* misclassified as a usage limit.
...
Before iteration 1, the runner probes every *candidate* agent (the ids reachable through
...
`ENABLE_CODE_SECURITY_REVIEW=0`) must reach *every* agent uniformly. The seam is
...
token appearing *inline* (e.g. quoted in prose) must not be mistaken for one.
