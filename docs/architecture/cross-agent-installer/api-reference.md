...
interface AgentTarget {
  /* configDirName, installSubdir, confidence, … */
}
interface DetectionResult {
  /* id, detected, configDirsProbed, … */
}
interface ResolveOpts {
  /* cwd / home overrides for resolution + tests */
}
interface InstallManifest {
  schemaVersion: 1; /* agent, scope, mode, files[]+sha256, … */
}
interface PlannedAction {
  /* kind: add|change|unchanged|remove|skip-modified, path, … */
}
interface RunReport {
  /* subcommand, scope, mode, dryRun, agents[], exitCode, raufError? */
}
```
