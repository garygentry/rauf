...
- Cross-module dependency: backlog.ts imports `readMarkerFile` from config.ts for smart defaults — acceptable within `@rauf/core`
...
- `@rauf/loop` must be built before web tests run — Vitest resolves workspace imports via `main: "dist/index.js"` field
...
- `@rauf/loop` added as CLI dependency + tsconfig reference `{ "path": "../loop" }`
...
- Must `pnpm --filter @rauf/core build` before `pnpm typecheck` in loop/cli/web packages — project references resolve via built `dist/` types
