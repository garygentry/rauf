...
- `--agent <id>`: coding-agent CLI that drives iterations (default: `claude-cli`). Supported built-ins: `claude-cli`, `codex`, `gemini`, `copilot`, `cursor`, `pi`, `generic-cli`. The id is the user-facing alias for the internal `provider` option; an unknown or absent agent is surfaced by the runner's fail-fast (no state written). For Pi, prefer `--agent pi --no-model` when backlog items may contain Claude-only model aliases. See [`rauf agents`](#rauf-agents) for live availability.
