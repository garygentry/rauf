...
any command inside it), and do NOT yield your turn to wait for an
asynchronous "completion notification" from a long-running test. Run
every command in the foreground, wait for it to exit **within this
turn**, then emit your exit signal in the SAME turn as your final
output. You run non-interactively: the session ends the moment you
yield your turn, so a deferred notification never arrives — no signal is
printed and the entire item is retried from scratch. (A backgrounded
verify command also exits immediately with no output, which looks like a
clean run and will be misread.)
<!-- rauf:managed:end -->
...
6. Run verification: \`{{verifyCommand}}\` (foreground, and wait for it to finish **within this turn** — never background it or defer your signal behind an async completion notification)
...
8. Output your exit signal on a line by itself, as your final line **in this same turn** (do not yield the turn first, then plan to signal later):
